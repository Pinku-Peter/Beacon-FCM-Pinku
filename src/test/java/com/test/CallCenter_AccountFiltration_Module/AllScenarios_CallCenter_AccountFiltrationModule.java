package com.test.CallCenter_AccountFiltration_Module;
...
public class AllScenarios_CallCenter_AccountFiltrationModule {
	...
	CallCenterAccountFiltrationPage callcenteraccountfiltrationPage;
    ...
    public void Allocated_To_dropdown_mandatory_checking() throws InterruptedException {
       ...
       //Click on the Search button
        ...
        wait.until(ExpectedConditions.invisibilityOfElementLocated(CallCenterAccountFiltrationRepo.warningmsg));
}
	...
	// Verify the warning message
     Assert.assertEquals(callcenteraccountfiltrationPage.getWarningMessageaftergivingvalueforallocatedto(), "Asset Category is Required");
        ...
        wait.until(ExpectedConditions.invisibilityOfElementLocated(CallCenterAccountFiltrationRepo.warningmsg2));
	 }
        catch (AssertionError | Exception e) {
			ExtentTestManager.getTest().log(Status.FAIL, "Test Failed: " + e.getMessage());
            throw e;
	...
	try {
     ...
     if (gridText.contains("No records to display.")) {
		...
		ExtentTestManager.getTest().log(Status.PASS, "Verified that the grid displays the message \"No records to display\" when no data is available.");
    	 ...
    	 Assert.fail("Test failed because grid text did not contain 'No records to display'. Instead, it contained: " + gridText);
     }
	}
		catch (AssertionError | Exception e) {
			ExtentTestManager.getTest().log(Status.FAIL, "Test Failed: " + e.getMessage());
    ...
    public void Enter_Allocation_Name_with_Alphabets(Map<Object, Object> testdata) throws InterruptedException {
        ...
        // Expected result: Allocation Name field accepts alphabets and no error shown
        // Assert: Can add further validations if needed, depending on the behavior
  ...
  public void Login_to_call_centre_application_and_take_account_filtration_pages() throws Exception {
      ...
      ExtentTestManager.getTest().log(Status.PASS, "User is navigated to Account Filtration page, URL shows CallCentre/CallCentreLeadFiltration.");
      wait.until(ExpectedConditions.invisibilityOfElementLocated(DispositionMasterPageRepo.spinner));
      wait.until(ExpectedConditions.visibilityOfElementLocated(CallCenterAccountFiltrationRepo.allocatedtodropdown));
}
...
//	
	 @Test(priority = 28)
	    public void Perform_Search_with_Valid_Datas() throws InterruptedException { 
		 ...
		 WebElement gridDataRow = wait.until(ExpectedConditions.visibilityOfElementLocated(CallCenterAccountFiltrationRepo.griddatarow));
		 boolean areResultsDisplayed = gridDataRow.isDisplayed();
	        ...
	        WebElement totalaccountselectedingrid = driver.findElement(CallCenterAccountFiltrationRepo.totalaccountselectedingrid);
	        WebElement totaloutstandingamountingrid = driver.findElement(CallCenterAccountFiltrationRepo.totaloutstandingamountingrid);
